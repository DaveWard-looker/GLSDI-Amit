view: security_bfa_pp {
  ...
  derived_table: {
    sql:
          ...
          FROM @{server_name_fire}.@{schema_name}.FOTC_DP_CE_GL_DETAIL_OUTPUT_PREVIOUS_PERIOD currency
          join @{server_name_fire}.@{schema_name}.FOTC_RD_MI_FLAT_DIMENSION_FOTC_BUSINESS_SEGMENT segment
          on currency.fotc_business_segment = segment.leaf
          join
          (select * from
         @{server_name_fire}.@{schema_name}.FOTC_RD_BFA_MI_FLAT_DIMENSIONS where  dimension ='Global_Standard_Product' and dimension_tree_id in ('WPB1', 'WPB2', 'WPB3', 'CMB1', 'GBM1','1')) product
      ...
      join
      (select * from
      @{server_name_fire}.@{schema_name}.FOTC_RD_BFA_MI_FLAT_DIMENSIONS where  dimension ='Global_Standard_Product' and dimension_tree_id in ('WPB1',  'CMB1')) product
      on currency.fotc_product= product.code
      ...
      join
      (select * from
      @{server_name_fire}.@{schema_name}.FOTC_RD_BFA_MI_FLAT_DIMENSIONS where  dimension ='Global_Standard_Product' and  dimension_tree_id in ('WPB1', 'WPB2', 'WPB3','1')) product
      on currency.fotc_product= product.code
      ...
      join
      (select * from
      @{server_name_fire}.@{schema_name}.FOTC_RD_BFA_MI_FLAT_DIMENSIONS where  dimension ='Global_Standard_Product' and dimension_tree_id in  ( 'WPB1','1')) product
      on currency.fotc_product= product.code
      where segment.level_1= 'GSS10001'
      ...
      join
      (select * from
      @{server_name_fire}.@{schema_name}.FOTC_RD_BFA_MI_FLAT_DIMENSIONS where  dimension ='Global_Standard_Product' and  dimension_tree_id in ('CMB1','1')) product
      on currency.fotc_product= product.code
      ...
      on currency.fotc_business_segment = segment.leaf
      join (select * from
      @{server_name_fire}.@{schema_name}.FOTC_RD_BFA_MI_FLAT_DIMENSIONS where  dimension ='Global_Standard_Product' and dimension_tree_id ='CMB1') product
      on currency.fotc_product= product.code
